...
import hust.project.gioimon.gm_post.service.utils.Validation;
...
public class PostFavouriteService {
    ...
    public synchronized Post favouritePost(PostFavourite postFavourite) {
        Long postId = postFavourite.getPostId();
        if (postService.getPost(postId) == null) {
            throw new RuntimeException();
        }
        long userId = postFavourite.getUserId();
        int point = postFavourite.getFavouritePoint();
        if (!Validation.validateFavouritePoint(point)) {
            return null;
