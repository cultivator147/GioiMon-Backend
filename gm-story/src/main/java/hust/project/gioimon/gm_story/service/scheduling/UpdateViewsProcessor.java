...
public class UpdateViewsProcessor extends Thread implements ScheduleProcessor {
    private final StoryRepository storyRepository;
    private final long delayTime = 30 * 60 * 1000;

    public void updateStoryViews() {
        System.out.println("Updating story's views");
        for(SampleStoryDTO story : ListStoryCache.LIST_STORIES) {
            ListStoryCache.updateViews(story.getId(), storyRepository.getViews(story.getId()));
        }
    }
